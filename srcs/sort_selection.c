...
void	insert_to_catcher(t_data *data, t_instr instr_set, int id)
{
	...
	if (id == S_A)
	{
		while (pos < data->stack[(id == S_A)].size && \
				stack_ud(data->stack[(id == S_A)], pos) > instr_set.next)
			pos++;
	}
	if (id == S_B)
	{
	...
	if (pos > data->stack[(id == S_A)].size / 2)
	{
		pos = data->stack[(id == S_A)].size - pos;
		instr_set.C1 = pos;
		instr_set.C2 = pos + 1;
		instr_set.fc = 1;
	}
	...
	if (instr_set.fc == instr_set.fp)
	{
		...
		exec_instr_loop(ROT_ID + instr_set.fc, S_BOTH, rot_two, data);
		if (instr_set.C1 > instr_set.P1)
			exec_instr_loop(ROT_ID + instr_set.fc, (id == S_A), instr_set.C1 - instr_set.P1, data);
		else
	...
	{
		exec_instr_loop(ROT_ID + instr_set.fp, id, instr_set.P1, data);
		exec_instr_loop(ROT_ID + instr_set.fc, (id == S_A), instr_set.C1, data);
	}
	exec_instr_loop(PUSH_ID, (id == S_A), 1, data);
	exec_instr_loop(ROT_ID + (instr_set.fc == 0), (id == S_A), instr_set.C2, data);
}
...
void	selection_sort(t_data *data, int id)
{
	...
	insertion_sort(next_table, data->n_ins, 0);
	i = 0;
	...
	free_index_table(index_table, data->stack[id].size);
	free_index_table(next_table, data->n_ins);
